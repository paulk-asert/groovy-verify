...
| File | Role |
|---|---|
| `VerifyChecker` | the `@TypeChecked` extension; call-site, body, loop & implicit checks, annotation-law synthesis (`@Reducer` / `@Monadic`), the information-flow noninterference walk (`@Label`), Bean Validation (`jakarta` / `javax.validation`) constraints read as preconditions (Phase 128), and the SEQ/PAR ladder passes (PAR interference over fork-join windows, channel-end linearity, channel element contracts, the network wait-for/well-foundedness analysis with close/drain discipline, guarantee-conformance twins, and the rungs past the one-shot fragment — bounded FIFO traffic, streaming, the ALT and its liveness, rely/guarantee cyclic streams, quantitative service bounds), plus the two disciplines that are not channel-shaped and so have no file of their own: `Phaser` BARRIERS (a sync after resigning, short party counts, and a knot closed by a barrier and a channel together) and the CREW read/write discipline (a write taken under the read lock, halves on different locks). Also drains the actor checks per method and, in `afterVisitClass`, the ones that belong to a class rather than a method |
...
| `ActorMailbox` | the ACTOR surface of `groovy.concurrent`, every rule of it MEASURED against the runtime before being modelled (`ActorMailboxSemanticsTest`, `ActorStashSemanticsTest`, `ActorReplySemanticsTest`, `ActorErrorSemanticsTest`, `ActorTimerSemanticsTest` — never assumed). The **bounded mailbox**: `withBoundedMailbox(k, Overflow)` names a burst that fills a BLOCK mailbox whose handler waits on the filler (a wait-for cycle), and a claim on a `sendAndGet` reply past the bound under a lossy policy. The **stash**: a stashed message returns only through `unstashAll()`, so an actor that never replays loses it, and a literal burst past `withStashBound(n, …)` overflows with its policy's measured outcome. The **become-graph** (`becomeGraph`) — the phases, their `if (m == LIT)` arms, a default that handles / stashes / throws / moves on, and the `onError` recovery edge — which `SessionChecker` plays an actor `@Protocol` role with. The **timers**: `scheduleOnce` / `scheduleAtFixedRate` land in whichever phase is current when they fire, so every phase reachable from the arming point is asked what it does with the message. And the **declaration forms**: an actor local to a method, or a field of the class — initialiser, constructor, instance initialiser or `static` block, with two competing definitions dropped rather than resolved by guessing — so `checkClass` raises the behaviour-graph findings once for the class while the send-dependent ones stay per method. Also exports the handler closure to `VerifyChecker`, which registers it as a PROCESS so a handler's channel waits are not mistaken for the caller's |
...
| `TrustLedger` | the trusted-spec ledger (Phase 216): one deduplicated inventory of every fact assumed without proof — in-place spec-only `@ThrowsIf` arms (`woven = false, direct = false`), external-spec registry consumption, and opaque `Monoid`/`Semigroup` carriers at parallel reductions (Phase 291) — surfaced in a consumer's own build by `VERIFY_TRUST=report|deny` (drained per class in `afterVisitClass`), printed beside the harness perf line, exposed via `entries()`/`summary()`, and backed by DocLint's `[5] trusted inventory` lint (a malformed shipped spec is silent trust loss, asserted as drift in `check`) |
