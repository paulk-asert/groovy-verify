...
| `TrustLedger` | the trusted-spec ledger (Phase 216): one deduplicated inventory of every fact assumed without proof — in-place spec-only `@ThrowsIf` arms (`woven = false, direct = false`), external-spec registry consumption, and opaque `Monoid`/`Semigroup` carriers at parallel reductions (Phase 291) — printed beside the harness perf line, exposed via `entries()`/`summary()`, and backed by DocLint's `[5] trusted inventory` lint (a malformed shipped spec is silent trust loss, asserted as drift in `check`) |
