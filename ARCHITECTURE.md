...
| `ActorMailbox` | Phase 289 — the BOUNDED ACTOR MAILBOX, the only send in `groovy.concurrent` besides a rendezvous channel that blocks its sender (measured in `ActorMailboxSemanticsTest`, not assumed). Reads `withBoundedMailbox(k, Overflow)` off an actor declaration and names two hazards: a burst that fills a BLOCK mailbox whose handler is waiting on the filler (a wait-for cycle), and a claim on a `sendAndGet` reply past the bound under a lossy policy (the reply carries an `IllegalStateException`, not an answer). Also exports the handler closure to `VerifyChecker`, which registers it as a PROCESS so a handler's channel waits are not mistaken for the caller's |
