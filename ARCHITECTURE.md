...
| `ThrowsIf` (+ `ThrowsIfConditions`) | the exceptional-contract annotation family (Phases 213/214, prototype): `@Repeatable` arms of `@ThrowsIf(value = { x == null }, exception = E)` — throws exactly when the condition holds. Per-arm modes: woven (the transform inserts the generative guard pre-STC — the reference weaving), woven=false (body-implemented; the checker proves the full iff, modelling `Objects.requireNonNull` as a guard-throw), trusted (spec-only: vacuity-checked, caller-assumed, rung-monitored, no proof and no warning). The rung reads the family reflectively and promotes justified throws to positive cross-validations |
...
| `TrustLedger` | the trusted-spec ledger (Phase 216): one deduplicated inventory of every fact assumed without proof — in-place `trusted` `@ThrowsIf` arms and external-spec registry consumption — printed beside the harness perf line, exposed via `entries()`/`summary()`, and backed by DocLint's `[5] trusted inventory` lint (a malformed shipped spec is silent trust loss, asserted as drift in `check`) |
