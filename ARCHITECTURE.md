...
| `EncodingPack` / `TheoryApi` / `PackRegistry` | the **extensible-encodings SPI** (experimental): a pack contributes a library/domain vocabulary's call recognisers + axioms from outside the core (packs model *libraries*; the core models the *language*). `TheoryApi` is the curated encoder facade packs program against (session, translate, `axiomsOnce`, the tri-state `NO_MATCH` convention shared with the built-in registry); `PackRegistry` discovers packs via `ServiceLoader` (`META-INF/services/verification.EncodingPack`) and the encoder dispatches them at a fixed registry slot |
