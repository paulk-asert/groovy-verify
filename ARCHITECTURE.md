...
| `ServedWithin` | Phase 265 — `@ServedWithin(n)`: the quantitative bound claim on the method's ALT, certified against a held `fair()`'s rotation arithmetic (n >= branches), refuted with the policy's own reason otherwise |
| `Protocol` / `SessionChecker` | Phase 263 — the `@Protocol` global type (Scribble-style messages, loops, choices) parsed, projected onto each role as a Thompson NFA over channel ops, each process's control flow read as an automaton over the same ops, conformance decided by inclusion on the product with the local type's subset-DFA, violations reported with their trace; `par { … } and { … }` projects as the shuffle of independent sub-sessions (Phase 264); structural, no solver |
