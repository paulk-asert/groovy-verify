...
| `ChannelGhosts` | Phase 259 — verification ghosts on channels as Groovy extension properties (registered via `META-INF/groovy/org.codehaus.groovy.runtime.ExtensionModule`) so a spec closure type-checks: `c.taken`, the elements the enclosing loop has taken from `c` so far, rewritten by the checker to the loop's taken-ghost; executing it throws |
