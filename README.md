...
fold (Phase 79). A plain `return [sum, product]` with `result[0]` / `result[1]` works the same way
(Phase 78); before either existed, this collapsed to `return s + p` with the weaker `result == sum +
product` (which only pins the two aggregates' *sum*, not each). The duck-typed `sum()` also covers
...
| **Recursive-sequence specs: `Fib.of(i)` / `Trib.of(i)`** | Fibonacci (`fib$`: base `0,1`, step `fib(k)=fib(k-1)+fib(k-2)`) and tribonacci/`fibfib` (`trib$`: base `0,0,1`, step `trib(k)=trib(k-1)+trib(k-2)+trib(k-3)`) spec helpers — the two-/three-term-recurrence siblings of `sum`/`prod`. The textbook iterative-equals-recursive proof verifies (`result == Fib.of(n)` / `result == Trib.of(n)`), the loop invariant carrying a 2- or 3-wide recurrence window re-established by the step axiom. These **are** HumanEval **055** (`fib`) and **063** (`fibfib`); 039's outer `prime_fib` search is a deliberate non-target (open-problem termination) | ✅ Phase 55 / 63 |
