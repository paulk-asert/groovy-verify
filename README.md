...
| **Nested loops (two levels)** | An annotated loop directly inside another annotated loop's body verifies **compositionally**, each loop cut by its own `@Invariant`/`@Decreases`. The outer loop's preservation/progress *summarise* the inner loop — havoc the variables it writes, then assume `inner_inv ∧ ¬inner_guard` — and the inner loop's own establishment / preservation / progress are discharged separately (establishment under `outer_inv ∧ outer_guard`; preservation **not** under `outer_inv`, which is generally false mid-inner-loop, e.g. `count == i*n` while count is changing). So `count = n*n` via a double loop, with `count == i*n` / `count == i*n + j`, proves end-to-end. Soundness rests on the summary being honest: a *too-weak* inner invariant can't sneak a false outer result through (the outer preservation fails because the summary leaves the accumulator unconstrained), an **un-annotated** inner loop or one whose body writes a **non-scalar** (array/collection) **skips loudly** rather than under-havoc, and three-deep nesting is out of the slice (skips loudly) | ✅ Phase 91 |
...
loop `while (i < n) a[i++] = 0` verifies and an out-of-bounds `a[i++]` refutes; Phases 85/86), and an
...
levels — see below). A `do … while` is `B; while (G) B` — its body runs once
