...
| **`**` power operator (first slice — typing + congruence)** | Z3 has no variable-exponent power, so `base ** exp` lowers to an uninterpreted `pow$ : (Int,Int) → Int` with no value axioms: `result == base ** exp` proves by congruence, while value properties (`2 ** n >= 1`, literal `2 ** 3 == 8`) stay "could not decide". Groovy's `**` returns `Number`, so the int surface is `(base ** exp).intValue()` — `.intValue()` / `.longValue()` are identity on the integral term. Value axioms (`pow(b,0)==1`, `pow(b,e)==b·pow(b,e-1)`) are a deliberate non-goal of the first slice (NIA-heavy, refute-hostile) | ✅ Phase 93 |
