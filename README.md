...
| **Reference identity + identity-keyed field reads** | Two object parameters of the same class are *alias-modelled*: their `int` fields read through a per-`(class, field)` map indexed by object identity, and `a === b` / `a.is(b)` lowers to identity equality `id(a) == id(b)`. So `a.is(b)` makes the fields **provably coincide** (`a.is(b) ⟹ a.balance == b.balance`), and the claim is **refuted without it** — reasoning the per-name model (distinct names ⇒ distinct objects) structurally cannot do. Read-only and `int`-field-only in this slice; field *writes* / the `transfer` mutator (verifies when disjoint, refutes under aliasing) are the next slice. Single-object-param and distinct-class methods keep the per-name model untouched | ✅ Phase 89 (slice 1) |
...
| **Recursive-sequence specs: `Fib.of(i)` / `Trib.of(i)` / `Gcd.of(a,b)`** | Fibonacci (`fib$`: base `0,1`, step `fib(k)=fib(k-1)+fib(k-2)`), tribonacci/`fibfib` (`trib$`: base `0,0,1`, step `trib(k)=trib(k-1)+trib(k-2)+trib(k-3)`), and Euclid's gcd (`gcd$`: base `∀x. gcd(x,0)=x`, step `∀x,y. y≠0 ⟹ gcd(x,y)=gcd(y, x%y)`) spec helpers — the two-/three-term-recurrence and two-*argument* siblings of `sum`/`prod`. The textbook iterative-equals-recursive proof verifies (`result == Fib.of(n)` / `Trib.of(n)`; `result == Gcd.of(a,b)` for Euclid's loop, whose invariant `Gcd.of(x,y) == Gcd.of(a,b)` is preserved by `t=x%y; x=y; y=t` via the step axiom and collapses at exit `y==0` through the base, terminating on the variant `y`). These **are** HumanEval **055** (`fib`), **063** (`fibfib`), and **013** (`greatest_common_divisor`); 039's outer `prime_fib` search is a deliberate non-target (open-problem termination). All three are **prove-friendly but refute-hostile** — e-matching certifies a true spec fast, but a *false* one (e.g. `Gcd.of(12,8)==5`) only soft-fails on a "could not decide / timeout" because finding a SAT model under an infinitely-instantiable axiom defeats MBQI (still sound — rejected, never a false pass). | ✅ Phase 55 / 63 / 87 |
...
| `Sets` / `Fib` / `Trib` / `Gcd` | runtime-executable spec helpers the encoder recognises, each lowered to an axiomatised primitive — `Sets.boundedBy`/`boundedCount` (cardinality), `Fib.of(i)` (Fibonacci), `Trib.of(i)` (tribonacci/`fibfib`), `Gcd.of(a, b)` (Euclid) |
