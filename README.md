...
- **[The Kerridge gallery](examples/kerridge.md)** — the CSP teaching shapes from Jon Kerridge's *Using Concurrency and Parallelism Effectively* (JCSP/groovyJCSP, the occam plugAndPlay vocabulary) ported to `groovy.concurrent` and certified: hello-world, GSquares, GPlus, GDelta, client-server — values proved, deadlock-freedom mechanised — with the classic student mistakes (the mutual-receive deadlock exercise, the missing poison pill) as *named compile errors*, and the streaming frontier stated honestly.
