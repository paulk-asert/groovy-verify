...
class Harvester {
    ...
    static final Map<String, String> GROUP_DESC = [
        ...
        'P161 each'                   : "Phase 161 — `xs.each { x -> body }` is modelled as the for-in `for (x in xs)`: stashed non-destructively on the call statement so the verifier finds it by metadata, with the auto bounds invariant (`0 <= idx <= size`) + `size - idx` variant proving SAFETY and termination without a hand-written @Invariant. A per-element property verifies, an unguarded one refutes, and the same relaxation lets a plain safety for-in verify with no invariant; an *accumulating* body (writing a variable the postcondition depends on) loud-skips — `.each` can't carry an @Invariant to frame it (a Groovy parse error on a method-call statement).",
