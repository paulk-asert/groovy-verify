...
class Harvester {
    ...
    static final Map<String, String> GROUP_DESC = [
        ...
        'P-range as-list'             : "A Groovy Range lo..hi is modelled as an immutable list of its elements (r[k] reads lo+k, size pinned): bare-range reads verify, a bare-range element write is refused (immutable, throws at runtime), the mutable copies [*lo..hi] / (lo..hi).toList() thread writes, and a whole-list result == [literal] folds to size + element-wise equality (a wrong element refutes).",
        'P-abs minvalue'              :"The Math.abs(Integer.MIN_VALUE) gotcha: abs's body `n < 0 ? -n : n` has `-n` overflow at MIN_VALUE under @CheckOverflow, so the `result >= 0` claim refutes (counterexample n = Integer.MIN_VALUE); excluding MIN_VALUE verifies.",
        'P-bloch binsearch'           :"Joshua Bloch's famous binary-search/mergesort midpoint overflow (a JDK bug hidden ~9 years): under @CheckOverflow the bug `(low + high).intdiv(2)` refutes with the counterexample mid(1, 2147483647), and his fix `low + (high - low).intdiv(2)` verifies.",
        'P-string-contract'           :"The Java-friendly String-contract prototype: this project's own verification.@Requires/@Ensures/@Decreases('…') take the condition as a String (a legal Java annotation value, unlike a closure) and capture it into the SAME reparse→encode→prove pipeline as a groovy-contracts closure. A recursive method verifies inductively (method-level @Decreases assumes the @Ensures at the recursive call) and a wrong @Ensures refutes on the base case; a straight-line method verifies; and a String @Requires discharges an implicit obligation (division-by-zero). Loop invariants stay out of reach — Java forbids statement annotations.",
        'P-check-then-act'            :"Check-then-act, the rung-1 boundary sharpened: a bounded counter whose `if (count < 1) count = count + 1` keeps `count <= 1` SEQUENTIALLY — groovy-verify proves the @Invariant (a wrong bound `<= 0` refutes) — yet is NOT thread-safe (two threads both pass the guard → count == 2, which jcstress's BoundedCounterJCStress catches, ~5 in 7 billion). The verifier reasons above the JMM, so it proves the SAME invariant for the racy and the @WithWriteLock-fixed version; only the structural rung tells them apart. A verified-sequential invariant that concurrency breaks — rung 1's documented scope, not unsoundness.",
        'P-philosophers'              :"Dining philosophers, the thread-local half of the deadlock-freedom story (the structural half is the Fray rung). Deadlock-free by *resource hierarchy*: if every philosopher acquires its two forks in increasing global index order, the wait-for graph is acyclic so no deadlock. groovy-verify proves that LOCAL ordering discipline as pure int arithmetic (min-first over the circle verifies for every philosopher; the discipline holds for any two distinct forks) and REFUTES the naive left-then-right scheme — the counterexample i=n-1 pinpointing the wrap-around philosopher whose acquisition closes the cycle.",
        'P-java-fragment'             :"Verifying a Java algorithm by treating it as Groovy: Java is largely a syntactic subset, so a typical integer/array method (semicolons, C-style `for`, typed locals, `return`, `new int[]{…}`, statement `@Invariant`/`@Decreases`) verifies unchanged once the contract annotations are added — a full Java-style `max` proves end-to-end. Every proof is over GROOVY semantics — the tool has no model of Java intent. The dangerous divergences that Groovy's own typing rejects are caught at COMPILE time: integer division `/` is BigDecimal (true) division, a hard @TypeChecked error when fed to an int (use `.intdiv()`), and the bare brace array initializer `{…}` is a closure that won't parse (use `[…]` or `new int[]{…}`). Divergences Groovy's typing tolerates are proved silently with Groovy semantics — `==` is value-equality (`.equals`), so two equal Strings verify `result == true`, which is NOT Java reference-equality: read the fragment as Groovy.",
